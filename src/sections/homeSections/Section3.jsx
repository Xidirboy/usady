...
import { babyIcon, cashIcon, dateIcon, footIcon, fromCIcon, hotelIcon, toCIcon, typeIcon, usersIcon } from '../../assets/homeS3Icon';
...
const Section3 = () => {
	...
	const [selectnutrition, SetSelectNutrition] = useState(null);
	useEffect(() => {
		getOptions();
	}, []);
	const onSubmit = e => {
			...
			if (user?.id) {
				Axios()
					.post('api/v1/application/create', {
						...
						to: selectto?.value,
						aircraft_class: selectClass?.value,
						hotel_rating: selectrating?.value,
						type_nutrition: selectnutrition?.value,
						children_count: sdata?.children_count ? sdata?.children_count : 0,
					})
					.then(r => {
						toast({
							title: 'Ваша заявка отправлена.',
							description: 'В ближайшее время вы получите предложения, соответствующие вашему заявлению.',
							status: 'success',
							duration: 9000,
							isClosable: true,
						});
					...
					<form className='app_form' onSubmit={onSubmit}>
						...
						<div className='fi50'>
							<InputUi
								...
								icon={footIcon}
								name='type_nutrition'
								value={selectnutrition}
								is_error={errors?.type_nutrition}
								thin_label={true}
								onChange={SetSelectNutrition}
